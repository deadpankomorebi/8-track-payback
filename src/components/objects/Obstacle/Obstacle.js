...
class Obstacle extends Group {
  constructor(parent, position) {
    ...
    let bigGeometry = new BoxBufferGeometry(
      this.state.frequencyNum / this.state.scaleFactor,
      1.5,
      1
    );
  ...
  checkCollision(player, freqData) {
    ...
    if (this.state.cube.position.y + 0.5 < player.position.y - 0.5)
      return false;
    if (this.state.cube.position.y - 0.5 > player.position.y + 0.5)
      return false;
    ...
    if (this.state.cube.position.x < 0) {
      // obstacles on the right side
      end =
        freqData / (this.state.scaleFactor * 2) + this.state.cube.position.x;
